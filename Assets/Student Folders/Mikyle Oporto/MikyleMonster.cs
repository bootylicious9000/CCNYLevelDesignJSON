...
public class MikyleMonster : HazardController
{
    public override void DoAction(string act, float amt = 0)
    {
        ...
        if (act == "DownSplitFire")
        {
            StartCoroutine(DownSplitFire());
        }
        if (act == "UpSplitFire")
        {
    ...
    public IEnumerator DownSplitFire()
    {
        //A simple timer
        float timer = 0;
    ...
    public IEnumerator UpSplitFire()
    {
        //A simple timer
        float timer = 0;
