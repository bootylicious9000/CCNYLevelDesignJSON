...
public class Ryan_ActorController : MonoBehaviour
{
    //My rigidbody
    ...
    public Ryan_ProjectileController DefaultProjectile;
    //Spawns this gnome (particle and audio emitter) when you shoot 
    public Ryan_GnomeScript ShootGnome;
    //Spawns this gnome (particle and audio emitter) when you die
    public Ryan_GnomeScript DeathGnome;
    ...
    void Start()
    {
        ...
        Ryan_GameManager.Singleton.AddActor(this);
    ...
    public virtual void TakeDamage(float amt, Ryan_ActorController source=null)
    {
    ...
    public virtual void Die(Ryan_ActorController source=null)
    {
    ...
    public float LookAt(Ryan_ActorController targ,float turnTime=0)
    {
    ...
    public void ShootAtPlayer(Ryan_ProjectileController prefab)
    {
        ShootAtPlayer(prefab,transform.position);
    }

    public void ShootAtPlayer(Ryan_ProjectileController prefab, Vector3 pos)
    {
        ShootAt(Ryan_PlayerController.Player.transform.position,prefab,pos);
    }
    ...
    public void ShootAt(GameObject targ, Ryan_ProjectileController prefab)
    {
        ShootAt(targ.transform.position,prefab,transform.position);
    }
    public void ShootAt(GameObject targ, Ryan_ProjectileController prefab, Vector3 pos)
    {
    ...
    public void ShootAt(Vector3 targ, Ryan_ProjectileController prefab)
    {
        ShootAt(targ,prefab,transform.position);
    }
    public void ShootAt(Vector3 targ, Ryan_ProjectileController prefab, Vector3 pos)
    {
    ...
    public void Shoot(Ryan_ProjectileController prefab=null)
    {
        Shoot(prefab,transform.position,transform.rotation.eulerAngles);
    }
    public void Shoot(Ryan_ProjectileController prefab, Vector3 pos)
    {
        Shoot(prefab,pos,transform.rotation.eulerAngles);
    }
    public void Shoot(Ryan_ProjectileController prefab,Vector3 pos, Vector3 rot)
    {
        //If I don't list a prefab, use my default projectile
        if (prefab == null) prefab = DefaultProjectile;
        if (prefab == null) return;
        //Spawn a bullet
        Ryan_ProjectileController p = Instantiate(prefab, pos, Quaternion.Euler(rot));
    ...
    private void OnDestroy()
    {
        ...
        Ryan_GameManager.Singleton.RemoveActor(this);
    }

    //This gets called by JSON
    public void TakeEvent(Ryan_EventJSON e)
    {
